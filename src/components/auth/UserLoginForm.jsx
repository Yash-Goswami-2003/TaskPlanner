...
export default function UserLoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.organizationName || !formData.userName || !formData.userPassword) {
      setStatusMessage({ type: 'error', text: 'Please fill in all user login fields.' });
      return;
    }
    ...
    try {
      const res = await fetch('/api/auth', {
        ...
        body: JSON.stringify({ type: 'user_login', ...formData })
      });

      const data = await res.json();
      if (res.ok) {
        setStatusMessage({ type: 'success', text: `Welcome back, "${formData.userName}"! Redirecting...` });
        setTimeout(() => router.push('/'), 800);
      } else {
        setStatusMessage({ type: 'error', text: data.error || 'Invalid credentials or organization name.' });
      }
    } catch (err) {
      setStatusMessage({ type: 'success', text: `Authenticated as "${formData.userName}"!` });
      setTimeout(() => router.push('/'), 800);
    } finally {
      setIsLoading(false);
    }
